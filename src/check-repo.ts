...
async function checkDefaultTemplateImages(ctx: Context, layoutsYaml: any): Promise<Result> {
  ...
  for (const item of layoutsYaml.items) {
    core.info(`checking item ${item.image_uri}`);
    const imageUri = path.join(ctx.repoPath, 'image', item.image_uri);
    try {
      const stats = await fs.promises.stat(imageUri);
      if (!lagestFile || stats.size > lagestFile.stats.size) {
        lagestFile = { file: item.image_uri, stats };
      }
    } catch (err) {
      const message = `error occured when checking ${item.image_uri}: ${err}`;
      core.error(message);
      errors.push({
        filename: item.image_uri,
        message,
      });
    }
  }
